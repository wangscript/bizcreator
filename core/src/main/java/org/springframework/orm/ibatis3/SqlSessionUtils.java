...
public final class SqlSessionUtils {
    ...
    public static SqlSession getSqlSession(SqlSessionFactory sessionFactory, DataSource dataSource,
        ...
        // note the DataSource will either be synchronized with the transaction
        ...
        if (holder == null) {
            ...
            if (shouldManageConnection(session, session.getConfiguration().getEnvironment().getDataSource())) {
                ...
                try {
                    ...
                    logger.warn("error closing SqlSession JDBC Connection", sqle);
                }
            }

            logger.debug("Closing non-transactional SqlSession");
            session.close();
        }
        else {
            holder.released();
    ...
    public static boolean shouldManageConnection(SqlSession session, DataSource dataSource) {
        ...
        if (usesManagedTransactions(configuration)) {
            if (DataSourceUtils.isConnectionTransactional(session.getConnection(), dataSource)) {
                return false;
            }
            else {
                return true;
    ...
    private static final class SqlSessionSynchronization extends TransactionSynchronizationAdapter {
        ...
        public SqlSessionSynchronization(SqlSessionHolder holder, SqlSessionFactory sessionFactory,
            ...
            this.manageConnection = usesManagedTransactions(holder.getSqlSession().getConfiguration())
                    && !DataSourceUtils.isConnectionTransactional(holder.getSqlSession().getConnection(), dataSource);
