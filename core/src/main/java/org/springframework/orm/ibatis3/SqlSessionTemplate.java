...
public class SqlSessionTemplate extends JdbcAccessor implements SqlSessionOperations {
    ...
    public <T> T execute(SqlSessionCallback<T> action, ExecutorType executorType) throws DataAccessException {
        ...
        try {
            ...
            throw getExceptionTranslator().translate("SqlSession operation", null, ex);
        }
        finally {
    ...
    public <T> T getMapper(Class<T> type) {
        return sqlSessionFactory.getConfiguration().getMapper(type,
                SqlSessionUtils.getSqlSession(sqlSessionFactory, getDataSource()));
    }
}
