    ...
    public void addToPico(MutablePicoContainer pico) {
        ...
        if (keyInfos != null) {
            for (int i = 0; i < keyInfos.length; i++) {
                Object key;
                Object value;
                if (keyInfos[i] instanceof BeanInfo) {
                    key = pico.getComponent(keyInfos[i].getName());
                }
                else if (keyInfos[i] instanceof RefInfo) {
                    key = pico.getComponent(((RefInfo)keyInfos[i]).bean);
                }
                else {
                    key = keyInfos[i].createObject(null);
                }
                
                if (valueInfos[i] instanceof BeanInfo) {
                    value = pico.getComponent(valueInfos[i].getName());
                }
                else if (valueInfos[i] instanceof RefInfo) {
                    value = pico.getComponent(((RefInfo)valueInfos[i]).bean);
                }
                else {
                    value = valueInfos[i].createObject(null);
                }
                map.put(key, value);
            }
        }
        pico.addComponent(hasName() ? getName() : Map.class, map);
    }
}
