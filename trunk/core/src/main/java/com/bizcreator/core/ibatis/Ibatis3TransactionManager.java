...
public final class Ibatis3TransactionManager extends DataSourceTransactionManager {
    ...
    protected void doBegin(Object transaction, TransactionDefinition definition) {
        ...
        if (session == null) {
            ...
            // doBegin()
            Connection connection = ((ConnectionHolder) TransactionSynchronizationManager.getResource(getDataSource())).getConnection();
    ...
    protected void doRollback(DefaultTransactionStatus status) throws TransactionException {
        ((SqlSession) TransactionSynchronizationManager.getResource(sessionFactory)).rollback();
    ...
    protected void doCleanupAfterCompletion(Object transaction) {
        SqlSession session = (SqlSession) TransactionSynchronizationManager.unbindResource(sessionFactory);
        session.close();
