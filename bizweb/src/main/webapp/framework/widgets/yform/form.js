...
Y.mix(Form, {
	...
	ATTRS : {
		 ...
		 * @type String
		 * @default 'post'
		 * @description The method by which the form should be transmitted.
		 ...
		 * @attribute fields
		 * @type Array
		 * @description An array of the fields to be rendered into the form.
		 *              Each item in the array can either be a FormField
		 *              instance or an object literal defining the properties of
		 *              the field to be generated. Alternatively, this value
		 ...
		 * @attribute errors
		 * @type Array
		 * @description An array of errors to be pre-set on form fields. Each
		 *              error is defined by an object literal containing the
		 *              properties 'name' (corresponding to a form field) and
		 *              'message'
		 */
		errors : {
			writeOnce : true,
...
Y.extend(Form, Y.Widget, {
	 ...
	 * @type Object
	 * @protected
	 * @description An object who's keys represent the IO
	 *              request ids sent by this Y.Form instance
	 */
	_ioIds : null,
	 ...
	 * @param {String}
	 *            val
	 * @description Validates the values of the 'action'
	 *              attribute
	 */
	 ...
	 * @param {Array}
	 *            val
	 * @description Validates the values of the 'fields'
	 *              attribute
	 */
	 ...
	 * @param {Array}
	 *            val
	 * @description Validates the value of the 'errors'
	 *              attribute
	 */
	_validateErrors : function(val) {
		...
		for (; i < l; i++) {
			if (!Y.Lang.isObject(val[i]) || !val[i].name
					|| !val[i].message) {
				valid = false;
				break;
			}
	 ...
	 * @param {Array}
	 *            fields
	 * @description Transforms the values passed to the 'fields'
	 *              attribute into an array of Y.FormField
	 *              objects
	 */
	 ...
	 * @param {Y.Node}
	 *            contentBox
	 * @description Sets the 'action' attribute based on parsed
	 *              HTML
	 */
	 ...
	 * @param {Y.Node}
	 *            contentBox
	 * @description Sets the 'method' attribute based on parsed
	 *              HTML
	 */
	 ...
	 * @param {Y.Node}
	 *            contentBox
	 * @description Sets the 'fields' attribute based on parsed
	 *              HTML
	 */
	_parseFields : function(contentBox) {
		var children = contentBox.all('*'), labels = contentBox
				.all('label'), fields = [];

		children
				.each(function(node, index, nodeList) {
					var nodeName = node.get('nodeName'), nodeId = node
							.get('id'), o, c = [];
					if (nodeName == 'INPUT') {
						o = {
							type : node.get('type'),
	...
	_renderFormNode : function() {
		var contentBox = this.get('contentBox'), form = contentBox
				.query('form');
	 ...
	 * @protected
	 * @description Syncs the form node action and method
	 *              attributes
	 */
	 ...
	 * @method _syncErrors
	 * @protected
	 * @description Syncs the form field errors with the defined
	 *              attribute
	 */
	 ...
	 * @protected
	 * @description Validates the form based on each field's
	 *              validator
	 */
	_runValidation : function() {
		var fields = this.get('fields'), i = 0, l = fields.length, isValid = true;
	 ...
	 * @param {Object}
	 *            ioResponse
	 * @description Handles the success event of IO transactions
	 *              instantiated by this instance
	 */
	 ...
	 * @param {Object}
	 *            ioResponse
	 * @description Handles the failure event of the IO
	 *              transactions instantiated by this instance
	 */
	...
	/**
	 * @method reset
	 * @description Resets all form fields to their initial
	 *              value and clears all errors
	 */
	reset : function() {
		this.clearErrors();
	...
	/**
	 * @method submit
	 * @description Submits the form using the defined method to
	 *              the URL defined in the action
	 */
	submit : function() {
		if (this._runValidation()) {
			var formAction = this.get('action'), formMethod = this
					.get('method'), formId = this._formNode
					.get('id'), cfg = {
				method : formMethod,
				form : {
					id : formId
	 ...
	 * @param {String |
	 *            Number} selector
	 * @description Get a form field by its name attribute or
	 *              numerical index
	 */
	...
	/**
	 * @method clearErrors
	 * @description Removes all the displayed errors on this
	 *              form
	 */
